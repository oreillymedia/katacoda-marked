...
Renderer.prototype.quiz = function(quizTokens) {
  ...
  for (var i = 0; i < quizTokens.items.length; i++) {
    ...
    if(e.type === "text") {
      body += "<input name='quiz-group-" + quizTokens.id + "' type='text' data-answer='" + (e.option || "") + "' data-value='" + e.text.toString("base64") + "'  />";
    } else {
      body += "<input name='quiz-group-" + quizTokens.id + "' type='" + e.type + "' data-answer='" + (e.option || "") + "' />" + e.text + "</br>";
    }
  }
